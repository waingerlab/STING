...
dir="H:/PCN/CM-PCN-wt-D0-050721-Stressors/sting/Data/"; 
...
///Identify dead cells from Hoescht image; make Hoescht and dead Hoescht mask 
			...
			run("Analyze Particles...", "size=50-500 circularity=0.3-1.00 show=Masks");
...
///////// STING Analysis: 
	...
	///Quantify STING intensity in cytobody and nucleus for CTIP2+ cells 

			/// Remove bright signal and measure intensity of STING in CTIP2+ nuclei 
			selectWindow("sting");
			...
			run("Options...", "iterations=10 count=1 black do=Dilate");
			run("Invert");
			imageCalculator("AND create","sting-b","ctip2pos-nucl-fin");
			run("Analyze Particles...", "size=21-infinity circularity=0.6-1.00 show=Masks");
			run("Invert LUT");	
			run("Options...", "iterations=15 count=1 black do=Dilate");
			run("Watershed");
			run("Duplicate...", "title=ctip2pos-nucl-1"); close("Result of sting-b");
			close("Mask of Result of sting-b");
	        ...
	        setThreshold(4000,65500); close("sting-a"); 

	        if(roiManager("Count") > 0){
			run("From ROI Manager");
			}
			roiManager("Measure");
			saveAs("Results", dir1+"data/sting_tuj/"+name+"_sting_ctip2_POSnucl_intensity_count.csv"); ///
			close(name+"_sting_ctip2_POSnucl_intensity_count.csv"); ///
			run("Clear Results");close("Summary");close("ROI Manager");

			close("sting-1"); close("ctip2pos-nucl-1");
			close("ctip2pos-nucl-2");

	///Quantify STING intensity in nucleus for CTIP2- cells 
			/// Remove bright signal and measure intensity of STING in CTIP2- nuclei 
			selectWindow("sting-b");
			imageCalculator("AND create","sting-b","ctip2neg-nucl-fin");
			run("Analyze Particles...", "size=21-infinity circularity=0.6-1.00 show=Masks");
			run("Invert LUT");	
			run("Options...", "iterations=15 count=1 black do=Dilate");
			run("Watershed");	
			run("Duplicate...", "title=ctip2neg-nucl-1"); close("sting-b"); close("Result of sting-b");
			...
			run("Duplicate...", "title=sting-1");
	        setThreshold(4000,65500); 

	        if(roiManager("Count") > 0){
			run("From ROI Manager");
			}
			roiManager("Measure");
			saveAs("Results", dir1+"data/sting_tuj/"+name+"_sting_ctip2_NEGnucl_intensity_count.csv"); ///
			close(name+"_sting_ctip2_NEGnucl_intensity_count.csv"); ///
			run("Clear Results");close("Summary");close("ROI Manager");
			close("sting-1"); close("ctip2neg-nucl-1");
			close("Mask of result of sting-b"); close("Result of sting");
...
///////////////// close all remaining windows 
			...
			close("Hoescht"); close("proj"); close("ctip2"); close("sting");
