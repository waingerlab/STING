...
dir="H:/PCN/CM-PCN-wt-D0-050721/pIRF3/Data/"; 
...
///MAKE a mask of TUJ + Hoescht = TOTAL CELL to remove non specific TUJ cells
  // to remove bright small object in the TUJ signal
	...
	//script to have cell bodies
			...
			run("Analyze Particles...", "size=12-infinity circularity=0.0-1.00 show=Masks");
			run("Invert"); run("Fill Holes"); run("Watershed"); //*
			run("Analyze Particles...", "size=12-infinity circularity=0-1.00 show=Masks");
...
///Identify dead cells and CTIP2+ cells from CTIP2 image		
			...
			run("Analyze Particles...", "size=35-infinity circularity=0.3-1.00 show=Masks");
...
/// Make final CTIP2+/- masks for nuclei
			...
			run("Analyze Particles...", "size=21-infinity circularity=0.3-1.00 show=Masks"); //20
			...
			run("Analyze Particles...", "size=21-infinity circularity=0.3-1.00 show=Masks");
...
///////// pIRF3 Analysis: 
	...
	///Quantify pIRF3 intensity in cytobody and nucleus for CTIP2+ cells 

			/// Remove bright signal and measure intensity of pIRF3 in CTIP2+ nuclei & cyto ("ctip2pos-body-fin" mask)	
			selectWindow("pIRF3");
			run("Duplicate...","title=pIRF3-b"); run("8-bit");
			setThreshold(180,255);
			run("Make Binary");
			run("Options...", "iterations=10 count=1 black do=Dilate");
			run("Invert");
			imageCalculator("AND create","pIRF3-b","ctip2pos-body-fin");
			run("Duplicate...", "title=ctip2pos-body-1"); close("Result of pIRF3-b");
			...
			selectWindow("pIRF3");
			run("Duplicate...","title=pIRF3-a");
			run("Gaussian Blur...", "sigma=100");
			imageCalculator("Subtract create","pIRF3","pIRF3-a");
			run("Duplicate...", "title=pIRF3-1");
	        setThreshold(350,65500); close("pIRF3-a"); 

	        if(roiManager("Count") > 0){
			run("From ROI Manager");
			}
			roiManager("Measure");
			saveAs("Results", dir1+"data/pirf3_tuj/"+name+"_pirf3_ctip2_POSbody_intensity_count.csv"); ///
			close(name+"_pirf3_ctip2_POSbody_intensity_count.csv"); ///
			run("Clear Results");close("Summary");close("ROI Manager");

			close("pIRF3-1"); close("ctip2pos-body-1");

	/// Measure intensity of pIRF3 in CTIP2+ nuclei ("ctip2pos-nucl-fin" mask)	
			selectWindow("ctip2pos-nucl-fin"); 
			run("Duplicate...","title=ctip2pos-nucl-1");
			run("Set Measurements...", "area mean standard min feret's integrated limit redirect=None decimal=2");
			run("Analyze Particles...", "size=0-infinity pixel circularity=0-1 show=Outlines add in_situ");

			selectWindow("Result of pIRF3");
			run("Duplicate...", "title=pIRF3-1");
	        setThreshold(350,65500); 

			if(roiManager("Count") > 0){
			run("From ROI Manager");
			}
			roiManager("Measure");
			saveAs("Results", dir1+"data/pirf3_tuj/"+name+"_pirf3_ctip2_POSnucl_intensity_count.csv"); ///
			close(name+"_pirf3_ctip2_POSnucl_intensity_count.csv"); ///
			run("Clear Results");close("Summary");
			close("ROI Manager");close("pIRF3-1"); close("ctip2pos-nucl-1");


	///Quantify pIRF3 intensity in cytobody and nucleus for CTIP2- cells 

			/// Remove bright signal and measure intensity of pIRF3 in CTIP2- nuclei & cyto ("ctip2neg-body-fin" mask)	
			selectWindow("pIRF3-b");
			imageCalculator("AND create","pIRF3-b","ctip2neg-body-fin");
			run("Duplicate...", "title=ctip2neg-body-1"); close("pIRF3-b"); close("Result of pIRF3-b");
			...
			roiManager("Measure");
			saveAs("Results", dir1+"data/pirf3_tuj/"+name+"_pirf3_ctip2_NEGbody_intensity_count.csv"); ///
			close(name+"_pirf3_ctip2_NEGbody_intensity_count.csv"); ///
			run("Clear Results");close("Summary");close("ROI Manager");
			close("pIRF3-1"); close("ctip2neg-body-1");

			/// Measure intensity of pIRF3 in CTIP2- nuclei ("ctip2neg-nucl-fin" mask)	
			selectWindow("ctip2neg-nucl-fin"); 
			run("Duplicate...","title=ctip2neg-nucl-1");
			run("Set Measurements...", "area mean standard min feret's integrated limit redirect=None decimal=2");
			run("Analyze Particles...", "size=0-infinity pixel circularity=0-1 show=Outlines add in_situ");
			...
			roiManager("Measure");
			saveAs("Results", dir1+"data/pirf3_tuj/"+name+"_pirf3_ctip2_NEGnucl_intensity_count.csv"); ///
			close(name+"_pirf3_ctip2_NEGnucl_intensity_count.csv"); ///
			run("Clear Results");close("Summary");
			close("ROI Manager");close("pIRF3-1"); close("ctip2neg-nucl-1");
			close("Result of pIRF3");
...
///////////////// close all remaining windows 
			...
			close("Hoescht"); close("proj"); close("ctip2"); close("pIRF3");
